...
bool createGPXFileFromJSON(char *json, char *fileName) {
  GPXdoc *obj = JSONtoGPX(json);
  bool isValid = validateGPXDoc(obj, "gpx.xsd");
  bool didWrite = false;
  if (isValid) {
    didWrite = writeGPXdoc(obj, fileName);
  }
  return didWrite;
}

char *getGPXJSON(char *fileName) {
  GPXdoc *obj = createValidGPXdoc(fileName, "gpx.xsd");
  char *gpxJSON = GPXtoJSON(obj);

  return gpxJSON;
}

char *getAllRoutesJSON(char *fileName)
{
    char *buf;
  	buf = malloc(sizeof(char*) * 500);

    char * strReturn;
    strReturn = malloc(sizeof(char*) * 500);

    char * directory = "uploads/";
    //put buf in upload directory
    strcpy(buf, directory);
	  strcat(buf, fileName);

    GPXdoc *obj = createGPXdoc(buf);
    bool isValid = validateGPXDoc(obj, "gpx.xsd");
    ...
    strReturn = routeListToJSON(obj->routes);

    return strReturn;
}
char *getAllTracksJSON(char *fileName)
{
    char *buf;
  	buf = malloc(sizeof(char*) * 500);
    ...
    strReturn = trackListToJSON(obj->tracks);

    return strReturn;
}

char *getWaypointsFromRoute(char *fileName, char * routeName)
{
    char *buf;
  	buf = malloc(sizeof(char*) * 500);
    ...
    pointIter->current = obj->routes->head;
    ...
    while((temp = nextElement(pointIter)) != NULL){
      ...
      if (strcmp(temp->name, routeName) == 0) {
        return waypointListToJSON(temp->waypoints);
      }
    }
...
bool renameRoute(char * fileName, char * routeName, int spot){
  char *buf;
  buf = malloc(sizeof(char*) * 500);
  ...
  ListIterator getter = createIterator(obj->routes);
  Route * temp = nextElement(&getter);

  int i = 0;
  while(temp != NULL){
    if(i == spot){
      strcpy(temp->name, routeName);
    }
    else{
      i++;
    }
    temp = nextElement(&getter);
  }

  bool write = writeGPXdoc(obj, buf);

  if(write == false){
    printf("Failed to change name of route\n");
    return false;
  }
  return true;
}
bool renameTrack(char * fileName, char * trackName, int spot){
  char *buf;
  buf = malloc(sizeof(char*) * 500);
  ...
  ListIterator getter = createIterator(obj->tracks);
  Track * temp = nextElement(&getter);

  int i = 0;
  while(temp != NULL){
    if(i == spot){
      strcpy(temp->name, trackName);
    }
    else{
      i++;
    }
  ...
  if(write == false){
    printf("Failed to change name of track\n");
    return false;
  }
  return true;
}
...
List* getTracksBetween(const GPXdoc* doc, float sourceLat, float sourceLong, float destLat, float destLong, float delta){
  ...
  for(int i = 0; i < trackLen; i++){
    ...
    // start is the first waypoint of the first segment

    TrackSegment *firstSegment = (TrackSegment *)temp->segments->head->data;

    Waypoint *firstWaypoint = (Waypoint *)firstSegment->waypoints->head->data;

    // destination is the last waypoint of the last segment
    TrackSegment *lastSegment = (TrackSegment *)temp->segments->tail->data;

    Waypoint *lastWaypoint = (Waypoint *)lastSegment->waypoints->tail->data;

    // check if provided points fall within the delta
    if(haver(sourceLong, firstWaypoint->longitude, sourceLat, firstWaypoint->latitude) <= delta){
      starter = 1;
    }
    //use dest for check dest long and lat
    if(haver(destLong, lastWaypoint->longitude,destLat, lastWaypoint->latitude) <= delta){
      ender = 1;
    }
  ...
  //   int trackSegLen = getLength(temp->segments);
	...
	// 		seg = (TrackSegment*)pointIter2->current->data;
...
char* waypointToJSON(const Waypoint *wpt){
  ...
  //route obj == null return empty JSON
	if(wpt == NULL && strcmp(wpt->name, "") == 0){
		sprintf(buf, "{}");
		return buf;
	}

  sprintf(buf, "{\"name\":\"%s\",\"latitude\":%.1f,\"longitude\":%.1f}", wpt->name, wpt->latitude, wpt->longitude);
...
char* waypointListToJSON(const List *list){
  ...
  //loop through then add comos
	while((temp = nextElement(listpoint)) != NULL){
		strcat(buf, waypointToJSON(temp));
		k++;
		if(k < i){
			strcat(buf, ",");
		}
